...
knwl.places.falsePlaces = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December', 'His', 'He', 'Her', 'Hers', 'Who', 'Whom', 'Whose', 'PM', 'AM', 'The'];
knwl.places.triggers = [['at'], ['in'], ['near'], ['close', 'to'], ['above'], ['below'], ['almost', 'to'], ['leaving'], ['arriving', 'at']];
knwl.places.findPlaces = function() {
    var words = knwl.words.linkWordsCasesensitive;

    // console.info('using search', knwl.tasks.search(knwl.places.triggers, words));

    var triggers = knwl.places.triggers;
    var places = [];
        ...
        if (isMatch) {
            ...
            if (word.length > 0 && word.length < 3) {
                ...
                for (var y = 0; y < word.length; y++) {
                    for (var x = 0; x < knwl.places.falsePlaces.length; x++) {
                        if (word[y] === knwl.places.falsePlaces[x]) {
                            isFalsePlace = true;
...
knwl.parserList.places = {
    calls: knwl.places.findPlaces
};